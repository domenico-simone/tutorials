...
The assembly stage could be memory- and time- consuming. Therefore it is recommended to run this tutorial by allocating a node (with either the `sbatch` or `interactive` commands from Rackham/UPPMAX).
...
We will use Roary to run pan-genome analysis. Our analysis will also include running a tool to build trees, Fasttree. 
...
roary -p 20 -f roary -e -n -v annotation/*.gff
```
...
Then we can plot the Roary results with `roary_plots.py`, a community contributed python script to visualise roary results. This script requires two non-standard python libraries: BioPython and Seaborn. The former is available as module on Rackham, the other one has to be installed with pip (a python package manager).
